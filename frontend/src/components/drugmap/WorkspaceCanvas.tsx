...
import { 
  ...
  MaximizeIcon, MinimizeIcon, AlertCircle, ChevronLeft, ChevronRight,
...
// Main workspace component - 從App.tsx中複製WorkspaceCanvas組件的全部內容
const WorkspaceCanvas = () => {
  ...
  */
  ...
  const addToHistory = (newComponents, newGroups) => {
  ...
  const addComponentToGroup = (componentId: string, groupId: string) => {
  ...
  const handleComponentDrag = (id, newX, newY) => {
    const newComponents = components.map(comp => 
      comp.id === id 
  ...
  const handleGroupDrag = (id, newX, newY) => {
    const newGroups = groups.map(group =>
      group.id === id
